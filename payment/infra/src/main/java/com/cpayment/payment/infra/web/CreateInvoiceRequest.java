...
import com.cpayment.payment.domain.model.NetworkConfirmations;
...
public record CreateInvoiceRequest(
    ...
    @Positive Integer minConfirmations
) {

    public CreateInvoiceCommand toCommand(String idempotencyKey) {
        AssetId parsedAsset = AssetId.parse(asset);
        int effectiveConfirmations = (minConfirmations != null)
            ? minConfirmations
            : NetworkConfirmations.defaultFor(parsedAsset);
